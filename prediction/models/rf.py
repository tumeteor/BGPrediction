...
from util.measures import computePerformanceMeals, mean_squared_error, mean_absolute_percentage_error
...
class RandomForest(BaseRegressor):
    ...
    def confidenceCal(self, train_data, test_data, predictions, test_y, rf):
        ...
        (_, caps, _) = plt.errorbar(predictions, test_y, yerr=np.sqrt(V_IJ), fmt='o', markersize=4, capsize=10, mfc='red',
         mec='green')
        for cap in caps:
            cap.set_markeredgewidth(1)
        plt.title('Error bars for Patient: ' + str(self.patientId))
        ...
        plt.savefig("prediction/tmp/confidence_intervals_bias_patient{}.png".format(self.patientId))
        plt.close()
    ...
    def predictWithData(self, data, y, _featureDesp="all"):
        ...
        if self.tune:
            ...
            # train_data, train_y = self.subData(train_data, train_y)
        rf.fit(train_data, train_y)
        ...
        #print "runtime for feature selection for patient{}. : {}".format(self.patientId, datetime.now() - start)
